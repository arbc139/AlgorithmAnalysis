...
public class LineIngredient {
	LineIngredient(int x_coeff, int y_coeff, int c_coeff) {
	 ...
	 */
	public static LineIngredient makeIngredient(Point p1, Point p2) {
		int x_coeff = p2.y - p1.y;
		int y_coeff = p1.x - p2.x;
		int c_coeff = (p2.x - p1.x) * p1.y - (p2.y - p1.y)*p1.x;
		return new LineIngredient(x_coeff, y_coeff, c_coeff);
	}
}
